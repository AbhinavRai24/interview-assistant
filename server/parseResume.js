...
async function parseResume(filePath) {
  ...
  if (ext === ".pdf") {
    ...
    try {
      ...
      console.warn(
        "pdf-parse require failed; falling back to safe empty text.",
        e && e.message
      );
      pdfParse = null;
    }
    ...
    if (pdfParse) {
      try {
        ...
        console.error(
          "pdf-parse failed to parse file:",
    ...
    try {
      ...
      console.error(
        "Failed to read uploaded file as text:",
        err && err.message ? err.message : err
      );
      text = "";
    }
